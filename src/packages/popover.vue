<template>
  <span>
    ...
    >
      ...
      >
        <div class="wz-popover__title" v-if="title" v-text="title"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
...
import { on, off, addClass, removeClass } from "../components/utils/dom";
...
export default {
  ...
  mounted() {
    ...
    // 可访问性
    if (reference) {
      addClass(reference, "wz-popover__reference");
